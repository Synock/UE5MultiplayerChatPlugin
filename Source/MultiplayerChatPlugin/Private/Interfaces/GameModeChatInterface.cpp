...
void IGameModeChatInterface::AreaSpeak(APlayerController* PlayerController, const FString& Message, float Range)
{
	...
	const bool OverlapStatus = UKismetSystemLibrary::SphereOverlapActors(GameMode->GetWorld(), PlayerController->GetPawn()->GetActorLocation(), Range,
																		 ObjectTypes, nullptr,
																		 ActorsToIgnore, ActorArray);

	UKismetSystemLibrary::DrawDebugSphere( GameMode->GetWorld(), PlayerController->GetPawn()->GetActorLocation(), Range, 8,
...
void IGameModeChatInterface::ShoutSpeak(APlayerController* PlayerController, const FString& String)
{
	...
	for(FConstPlayerControllerIterator PCIterator = GameMode->GetWorld()->GetPlayerControllerIterator(); PCIterator != EndIterator; ++PCIterator)
	{
